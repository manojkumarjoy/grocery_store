...
app.post('/login', async (req, res) => {
    ...
    if (isAdmin) {
        const adminToken = jwt.sign({ userId: 'admin', isAdmin: true }, 'mysecretkey'); // replace 'mysecretkey' with your environment variable
        return res.json({ jwtToken: adminToken });
    }
    ...
    const token = jwt.sign({ userId: user._id, isAdmin: false }, 'mysecretkey');
    res.json({ user, token });
});
